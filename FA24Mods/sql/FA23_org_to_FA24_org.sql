...
-- 2017-10-31
-- This script synchronises a standard FA 2.3.26 Chart to FA 2.4.2+
-- USE `23org`;
...
ALTER TABLE `0_cust_branch` 
	...
	DROP COLUMN `disable_trans`, 
	DROP KEY `branch_code`, ENGINE=InnoDB; 
...
CREATE TABLE `0_reflines`(
	...
	`prefix` char(5) COLLATE latin1_general_ci NOT NULL  DEFAULT '' , 
...
ALTER TABLE `0_stock_moves` 
	...
	CHANGE `reference` `reference` char(40) NOT NULL DEFAULT '' after `price`, 
...
ALTER TABLE `0_users` 
	ADD COLUMN `transaction_days` smallint(6)   NOT NULL DEFAULT 30 after `startup_tab`, 
	ADD COLUMN `save_report_selections` smallint(6)   NOT NULL DEFAULT 0 after `transaction_days`, 
	ADD COLUMN `use_date_picker` tinyint(1)   NOT NULL DEFAULT 1 after `save_report_selections`, 
	ADD COLUMN `def_print_destination` tinyint(1)   NOT NULL DEFAULT 0 after `use_date_picker`, 
	ADD COLUMN `def_print_orientation` tinyint(1)   NOT NULL DEFAULT 0 after `def_print_destination`, 
...
ALTER TABLE `0_wo_requirements` 
	...
	CHANGE `loc_code` `loc_code` char(5) NOT NULL DEFAULT '' after `unit_cost`, 
